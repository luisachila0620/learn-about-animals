import React from "react";
import "../../styles/home.scss";

export const Aquatic = () => (
	<div className="card-group">
		<div className="card">
			<img
				src="https://cdn.theatlantic.com/thumbor/2ZLu91qIpPiwnxz0I_H9LeJWJzk=/1x520:6177x3994/1600x900/media/img/mt/2019/04/GettyImages_611541996/original.jpg"
				className="card-img-top"
				alt="..."
			/>
			<div className="card-body">
				<h5 className="card-titles">shark</h5>
			</div>
		</div>
		<div className="card">
			<img
				src="https://www.bhmpics.com/download/underwater_largest_whale-1600x900.jpg"
				className="card-img-top"
				alt="..."
			/>
			<div className="card-body">
				<h5 className="card-titles">blue whale</h5>
			</div>
		</div>
		<div className="card">
			<img
				src="https://www.bhmpics.com/download/orca_whale_love-1600x900.jpg"
				className="card-img-top"
				alt="..."
			/>
			<div className="card-body">
				<h5 className="card-titles">killer whale</h5>
			</div>
		</div>
		<div className="card-group">
			<div className="card">
				<img
					src="https://img.wallpapersafari.com/desktop/1600/900/83/69/ckMzdG.jpg"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">Barracuda</h5>
				</div>
			</div>
			<div className="card">
				<img
					src="https://www.teahub.io/photos/full/202-2021548_red-fish-wallpaper.jpg"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">red fish</h5>
				</div>
			</div>
			<div className="card">
				<img src="https://eskipaper.com/images/manatee-1.jpg" className="card-img-top" alt="..." />
				<div className="card-body">
					<h5 className="card-titles">manatie</h5>
				</div>
			</div>
		</div>
		<div className="card-group">
			<div className="card">
				<img
					src="https://eskipaper.com/images/sea-turtle-wallpaper-13.jpg"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">turtle</h5>
				</div>
			</div>
			<div className="card">
				<img
					src="https://coolwallpapers.me/picsup/5735961-manta-ray-wallpapers.jpg"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">manta ray</h5>
				</div>
			</div>
			<div className="card">
				<img
					src="https://e3.365dm.com/20/06/1600x900/skynews-the-seahorse-trust_5004387.jpg?20200602143036"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">sea horse</h5>
				</div>
			</div>
		</div>
		<div className="card-group">
			<div className="card">
				<img src="https://images6.alphacoders.com/419/419848.jpg" className="card-img-top" alt="..." />
				<div className="card-body">
					<h5 className="card-titles">seal</h5>
				</div>
			</div>
			<div className="card">
				<img
					src="https://www.desktopbackground.org/download/1600x900/2012/11/27/490132_piranha-fish-photo-and-wallpaper-cute-piranha-fish-pictures_2398x1370_h.jpg"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">Piranha</h5>
				</div>
			</div>
			<div className="card">
				<img
					src="https://d.newsweek.com/en/full/1799781/bull-shark.jpg?w=1600&h=900&q=88&f=28a4f4e02e69247e33bbed09aef260d5"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">bull shark</h5>
				</div>
			</div>
		</div>
		<div className="card-group">
			<div className="card">
				<img
					src="https://e3.365dm.com/17/12/1600x900/skynews-tiger-shark-generic_4172004.jpg?20171202103900"
					className="card-img-top"
					alt="..."
				/>
				<div className="card-body">
					<h5 className="card-titles">tiger shark</h5>
				</div>
			</div>
			<div className="card">
				<img src="https://vistapointe.net/images/squid-7.jpg" className="card-img-top" alt="..." />
				<div className="card-body">
					<h5 className="card-titles">squid</h5>
				</div>
			</div>
			<div className="card">
				<img src="https://cdn.hswstatic.com/gif/sea-snake-1.jpg" className="card-img-top" alt="..." />
				<div className="card-body">
					<h5 className="card-titles">sea snake</h5>
				</div>
			</div>
		</div>
	</div>
);
